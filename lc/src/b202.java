...
// ##📘Giải thích thuật toán từ code Java của bạn
...
// Ta duyệt từ`1→n`Mỗi số`i`sẽ được nối vào cuối kết quả trước đó bằng cách:

// ```res=(res<<số_bit_của_i)+i```

// ---

// ##🔥Điểm thông minh trong code của bạn

// ###1 ️⃣Kiểm tra khi nào tăng số bit

// ```java if((i&(i-1))==0)bits++;```

// ###Vì sao điều kiện này đúng?

// Biểu thức:

// ```i&(i-1)```

// ✔️ Chỉ bằng 0 khi`i`là**lũy thừa của 2**

// Ví dụ:

// |i|Binary|i-1|i&(i-1)||-|------|----|---------||1|1|0|0||2|10|01|0||4|100|011|0||8|1000|0111|0|

// 👉Mỗi khi gặp số dạng`2^k`thì:

// *Số bit tăng thêm 1

// Ví dụ:

// *1→1 bit*2→2 bit*4→3 bit*8→4 bit

// ---

// ##2 ️⃣Nối nhị phân bằng bit shift

// ```java res=((res<<bits)+i)%mod;```

// Giải thích:

// *`res<<bits`→dịch trái bits lần→tương đương nhân`2^bits`→tạo chỗ trống

// *`+i`→thêm i vào phần trống đó
...
public class b202 {
    ...
    public static int concatenatedBinary(int n) {
        ...
        long res = 0; // lưu kết quả hiện tại
        long mod = 1000000007; // số mod theo đề bài
        int bits = 0; // số bit của số hiện tại

        for (int i = 1; i <= n; i++) {

            // Nếu i là lũy thừa của 2
            // thì số bit tăng thêm 1
            // (i & (i - 1)) == 0 chỉ đúng khi i là 2^k
            if ((i & (i - 1)) == 0) {
                bits++;
            }

            // Dịch trái bits lần để tạo chỗ
            // Sau đó cộng i để nối nhị phân
            res = ((res << bits) + i) % mod;
        }
...
// So sánh
// cách này
// với cách dùng`Integer.toBinaryString`*🔥
// Hoặc tối
// ưu thêm
...
// # 2. Nối (concatenate) tất cả các số nhị phân đó lại thành **một chuỗi duy
// nhất**.
