...
public class b227 {
    ...
    public static boolean areSimilar(int[][] mat, int k) {
        ...
        // Dịch k lần tương đương k % n
        k = k % n;

        // Nếu không dịch → luôn giống
        if (k == 0)
            return true;
        ...
        for (int i = 0; i < m; i++) {
            ...
            for (int j = 0; j < n; j++) {
                ...
                // So sánh phần tử hiện tại với phần tử
                // ở vị trí sau khi "dịch vòng"
                // (j - k + n) % n để tránh số âm
                if (mat[i][j] != mat[i][(j - k + n) % n]) {
                    return false; // chỉ cần sai 1 chỗ → false
                }
            }
        }

        return true; // tất cả đều đúng → true
    }
}

// // Mình sẽ giải thích**thuật toán**rồi viết lại code Java của bạn với:

// // ✅chú thích rõ ràng✅thêm`main`dùng`Scanner`(đúng style bạn thích)

// // ---

// // ##🧠Ý tưởng thuật toán(bản Java của bạn)

// // Bài Matrix Similarity After Cyclic Shifts yêu cầu:

// // 👉Sau khi**dịch vòng từng hàng k bước**,ma trận có giống ban đầu không?

// // ---

// // ###🔑Insight quan trọng

// // Không cần dịch thật❌Chỉ cần kiểm tra:

// // [mat[i][j]==mat[i][(j-k+n)%n]]

// // 👉Ý nghĩa:

// // *`(j-k+n)%n`=vị trí**trước khi bị dịch sang j***Nếu mọi phần tử khớp→ma
// trận không đổi

// // ---

// // ##💻Code Java(có chú thích+main)
...
// dùng (j - k)** chứ không phải `(j + k)`
// * Hoặc vẽ minh họa để bạn hiểu sâu

// hơn (rất dễ nhớ cho phỏng vấn)

// // # Mình sẽ giải thích **logic thuật toán** trước, rồi viết lại code của bạn
// với **chú thích chi tiết từng dòng** nhé.

// // # ---

// // # ## 🧠 Ý tưởng thuật toán

// // # 👉 Điều kiện để **sau khi cyclic shift mà ma trận vẫn giống ban đầu**
// là:
...
// // # # 👉 **Xoay (shift) từng hàng hoặc từng cột theo kiểu vòng tròn (cyclic
// shift)**
// // # # 👉 Sau đó kiểm tra xem **ma trận mới có giống ma trận ban đầu hay
// không**
