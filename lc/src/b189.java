...
// // 1.✅Giải thích thuật toán Java 2.✅Thêm chú thích chi tiết vào code 3.✅Viết thêm`main()`dùng`Scanner`4.✅Viết luôn class`TreeNode`đầy đủ
...
// // Ta duyệt toàn bộ đường đi từ**root→leaf**.
...
// // *Nhân 2=dịch trái trong hệ nhị phân*Cộng`rt.val`(0 hoặc 1)để thêm bit mới

// // Ví dụ đường đi:1→0→1

// Tính dần:

// ```0→0*2+1=1→1*2+0=2→2*2+1=5```
...
// // ```java if(rt.left==rt.right)```

// // Vì:

// // *Nếu node lá→left=null*Và right=null*Nên left==right(đều null)
...
// // Giả sử input theo dạng level-order Ví dụ:
...
public class b189 {
    ...
    // Hàm DFS duyệt cây
    // rt: node hiện tại
    // x: giá trị nhị phân tính đến node hiện tại
    public static int DFS(TreeNode rt, int x) {

        // Nếu node rỗng → không cộng gì
        if (rt == null)
            return 0;

        // Dịch trái (nhân 2) rồi cộng giá trị node
        x = x * 2 + rt.val;

        // Nếu là node lá
        // left == null và right == null
        if (rt.left == rt.right)
            return x;

        // Nếu chưa phải lá
        // Cộng kết quả từ nhánh trái và phải
...
// * Viết lại theo style **while thay cho for**
// * Hoặc tối ưu để không dùng Queue
// * Hoặc giải thích sâu tại sao không cần backtracking
...
// # Ta dùng **DFS (Depth-First Search)** để duyệt tất cả đường đi từ root →
// leaf.
...
// # Vì lúc đó ta đã tạo xong **1 số nhị phân hoàn chỉnh**, nên trả về giá trị
// đó để cộng vào tổng.
...
// # * Mỗi đường đi từ **root → leaf** tạo thành **một số nhị phân (binary
// number)**.
// # * Yêu cầu: **Tính tổng tất cả các số nhị phân đó (sau khi đổi sang hệ thập
// phân).**
...
// # Mỗi đường đi từ root đến leaf giống như bạn ghép các số 0/1 lại thành một
// số nhị phân.
