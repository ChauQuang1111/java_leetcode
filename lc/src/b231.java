...
//         // Sắp xếp task theo:
//         // (minimum required - actual cost)
//         // Task nào cần "dự trữ energy" nhiều hơn sẽ làm trước
//         Arrays.sort(tasks, (a, b) -> (b[1] - b[0]) - (a[1] - a[0]));

//         // energy = lượng energy ban đầu nhỏ nhất cần có
//         int energy = 0;

//         // curr = energy hiện tại đang có
//         int curr = 0;

//         // Duyệt từng task
//         for (int[] task : tasks) {

//             // task[0] = energy bị mất sau khi làm
//             int actual = task[0];

//             // task[1] = energy tối thiểu cần có để bắt đầu
//             int minimum = task[1];

//             // Nếu energy hiện tại chưa đủ để làm task
//             if (curr < minimum) {

//                 // Cộng thêm phần còn thiếu
//                 energy += minimum - curr;

//                 // Sau khi cộng thì curr đủ bằng minimum
//                 curr = minimum;
//             }

//             // Làm task xong -> bị mất actual energy
//             curr -= actual;
//         }

//         // Trả về energy ban đầu nhỏ nhất cần có
//         return energy;
//     }
// }
// ```
...
// => chỉ cần bắt đầu với 8 energy là đủ hoàn thành mọi task.
...
public class b231 {
    ...
    public static int minimumEffort(int[][] tasks) {
        ...
        for (int[] task : tasks) {
            ...
            // Nếu energy hiện tại chưa đủ
            if (curr < minimum) {

                // Cộng thêm phần energy còn thiếu
                energy += minimum - curr;

                // Sau khi cộng -> curr đủ bằng minimum
                curr = minimum;
            }

            // Làm task xong -> mất actual energy
            curr -= actual;
        }

        // Trả về energy nhỏ nhất cần có ban đầu
        return energy;
    }
}
