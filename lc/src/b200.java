...
public class b200 {
    ...
    // Hàm tính số bước để giảm số nhị phân về 1
    public static int numSteps(String s) {

        int n = s.length(); // Độ dài chuỗi nhị phân
        int carry = 0; // Biến nhớ khi cộng 1
        int res = 0; // Tổng số bước

        // Duyệt từ bit cuối (LSB) về bit thứ 2 (không xử lý bit đầu tiên)
        for (int i = n - 1; i > 0; i--) {

            // Giá trị thực tế của bit sau khi cộng carry
            // s.charAt(i) - '0' chuyển '0'/'1' thành 0/1
            if (s.charAt(i) - '0' + carry == 1) {

                // Trường hợp này nghĩa là số hiện tại là số lẻ
                // Theo đề:
                // lẻ -> +1 (1 bước)
                // sau đó /2 (1 bước)
                // Tổng = 2 bước
        ...
        // Nếu carry vẫn còn = 1,
        // nghĩa là có thêm một bước xử lý cuối cùng
        return res + carry;
    }
