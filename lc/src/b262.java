...
// Dưới đây là phần giải thích thuật toán, thêm hàm main sử dụng Scanner để nhập dữ liệu và chú thích chi tiết vào từng dòng code.
...
// Đếm xem có bao nhiêu chuỗi trong patterns xuất hiện trong word dưới dạng substring.
...
// Biến count dùng để lưu số lượng chuỗi xuất hiện trong word.

// Bước 2: Duyệt qua từng chuỗi trong patterns

// for(int i = 0; i < n; i++)

// Lần lượt lấy từng chuỗi:
...
// Hàm contains() của Java trả về:



// true nếu patterns[i] xuất hiện trong word
...
// word.contains("bc");    // true

// word.contains("abc");   // true

// word.contains("d");     // false

// Nếu là true thì tăng biến đếm:
...
public class b262{
    ...
    public static void main(String[] args) {
     int n = sc.nextInt();
        ...
        sc.nextLine(); // Xóa ký tự xuống dòng còn lại
        // Khai báo mảng patterns
    ...
    // Hàm đếm số chuỗi trong patterns xuất hiện trong word

    public  static int numOfStrings(String[] patterns, String word) {



        // Lấy số lượng phần tử của mảng patterns

        int n = patterns.length;



        // Biến đếm số chuỗi xuất hiện

        int count = 0;



        // Duyệt từng chuỗi trong patterns

        for (int i = 0; i < n; i++) {



            // Kiểm tra patterns[i] có phải là substring của word hay không

            if (word.contains(patterns[i])) {



                // Nếu có thì tăng biến đếm
...
// ipatterns[i]contains()count0atrue11abctrue22bctrue33dfalse3Output

// So chuoi xuat hien trong word = 3
...
// n là số chuỗi trong patterns.

// m là độ dài của word.

// k là độ dài trung bình của mỗi chuỗi trong patterns.

// Vòng lặp chạy n lần.

// Mỗi lần gọi word.contains(patterns[i]), Java tìm kiếm chuỗi con trong word, có độ phức tạp xấu nhất khoảng O(m × k).

// Do đó:



// Độ phức tạp thời gian: O(n × m × k) (với giới hạn của bài là tối đa 100 nên hoàn toàn đáp ứng).

// Độ phức tạp không gian: O(1), vì chỉ sử dụng một vài biến (n, count) và không tạo thêm cấu trúc dữ liệu phụ thuộc vào kích thước đầu vào.