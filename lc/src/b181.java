// // # # Reverse Bits (16/02/2026)

// Ok,mình sẽ giải thích**thuật toán bài 190. Reverse Bits(Java)**theo kiểu dễ hiểu+chú thích code chi tiết👇

import java.util.Scanner;

public class b181 {
    static Scanner sc = new Scanner(System.in);

    public static void main(String[] args) {
        int n = sc.nextInt();

        // Gọi hàm reverseBits
        int reversed = reverseBits(n);

        // In kết quả
        System.out.println(reversed);

        sc.close();
    }

    // Hàm đảo ngược 32 bit của số n
    public static int reverseBits(int n) {

        int result = 0; // Biến lưu kết quả sau khi đảo bit

        // Lặp đúng 32 lần vì số int có 32 bit
        for (int i = 0; i < 32; i++) {

            result <<= 1;
            /*
             * Dịch trái result 1 bit
             * Ví dụ: 101 -> 1010
             * Mục đích: tạo chỗ trống bên phải để gắn bit mới
             */

            result |= (n & 1);
            /*
             * n & 1 → lấy bit cuối của n (0 hoặc 1)
             * 
             * Ví dụ:
             * n = 10110 → n&1 = 0
             * n = 10111 → n&1 = 1
             * 
             * Dùng OR để gắn bit này vào result
             */

            n >>>= 1;
            /*
             * Dịch phải n 1 bit (unsigned shift)
             * 
             * >>> khác >> ở chỗ:
             * - >>> luôn chèn 0 bên trái
             * - Phù hợp với bài unsigned 32-bit
             * 
             * Sau bước này ta bỏ bit cuối vừa xử lý
             */
        }

        return result; // Trả về số sau khi đảo bit
    }
}

// ---

// #🧠Ý tưởng thuật toán

// Ta cần:

// >Đảo thứ tự 32 bit của số nguyên`n`

// Ví dụ:

// ```n=00000010100101000001111010011100 rev=00111001011110000010100101000000```

// Cách làm:

// *Lấy bit phải nhất của`n`*Gắn vào bên trái của`result`*Lặp 32 lần

// ---

// #🧾Code đã chú thích

// ```java
// class Solution {
// public int reverseBits(int n) {

// int result = 0; // Biến lưu kết quả sau khi đảo bit

// for (int i = 0; i < 32; i++) { // Lặp đúng 32 bit

// result <<= 1;
// // Dịch trái result 1 bit
// // -> Tạo chỗ trống bên phải để gắn bit mới

// result |= (n & 1);
// // Lấy bit cuối của n rồi gắn vào result
// // n & 1 → trả về 0 hoặc 1

// n >>>= 1;
// // Dịch phải n 1 bit (unsigned shift)
// // -> Bỏ bit cuối vừa xử lý
// }

// return result;
// }
// }```

// ---

// #🔍

// Giải thích
// từng phép toán

// ---

// ##1 ️⃣`n&1`—
// Lấy bit
// cuối

// Ví dụ:

// ```n=10110 n&1=0

// n=10111 n&1=1```

// 👉
// Dùng để“bóc”
// bit từ
// phải sang trái.

// ---

// ##2 ️⃣`result<<=1`—
// Dịch trái

// Ví dụ:

// ```result=101<<1=1010```

// Ý nghĩa:

// *
// Nhân đôi số*
// Tạo chỗ
// trống để
// gắn bit mới

// ---

// ##3 ️⃣`result|=(n&1)`—
// Gắn bit

// Ví dụ:

// ```result=1010 lastBit=1

// 1010 OR 0001=1011```

// ---

// ##4 ️⃣`n>>>=1`—
// Dịch phải
// KHÔNG dấu

// Đây là
// điểm quan
// trọng trong Java⚠️

// Có 2
// kiểu shift phải:

// |
// Toán tử|
// Ý nghĩa||-------|--------------------||`>>`|
// Shift phải
// có dấu||`>>>`|
// Shift phải
// không dấu|

// ---

// ###❓
// Vì sao dùng`>>>`?

// Vì đề bài:

// >
// n là số**unsigned 32-bit**

// Nếu dùng`>>`:

// *

// Bit dấu (sign bit) có thể bị giữ lại
// * Làm sai kết quả khi n âm

// `>>>` sẽ:

// * Luôn chèn `0` bên trái
// * Đúng với logic bit unsigned

// ---

// # 🔄 Minh họa thuật toán

// Giả sử (8 bit cho dễ nhìn):

// ```
// n = 00010110
// ```

// ### Lần 1

// ```
// result = 0 << 1 = 0
// lastBit = 0
// result = 0
// n >>> 1 → 00001011
// ```

// ### Lần 2

// ```
// result = 0 << 1 = 0
// lastBit = 1
// result = 1
// n >>> 1 → 00000101
// ```

// ### Lần 3

// ```
// result = 1 << 1 = 10
// lastBit = 1
// result = 11
// ```

// …

// Sau 8/32 lần:

// ```
// 01101000
// ```

// → Đã đảo bit.

// ---

// # 📌 Tóm tắt thuật toán

// **Input:** số nguyên 32 bit `n`

// **Process:**

// Lặp 32 lần:

// 1. Dịch trái `result`
// 2. Lấy bit cuối `n & 1`
// 3. Gắn vào `result`
// 4. Dịch phải `n`

// **Output:** số đã đảo bit

// ---

// # ⏱️ Độ phức tạp

// * Time: `O(32)` → coi như `O(1)`
// * Space: `O(1)`

// ---

// # 💡 Nhớ nhanh 1 câu

// > Bóc bit phải của n → nhét sang trái result → lặp 32 lần

// ---

// Nếu bạn muốn mình giải thích thêm:

// * So sánh `>>` vs `>>>` bằng ví dụ âm
// * Trace full 32 bit đề LeetCode
// * Cách tối ưu khi gọi nhiều lần

// Cứ nói mình biết 👍

// # Ok, mình sẽ **chú thích trực tiếp vào code** + giải thích thuật toán theo
// kiểu dễ hiểu nhất 👇

// # ---

// # ## 🧾 Code đã chú thích

// # ```python
// class Solution:
// def reverseBits(self, n: int) -> int:
// result = 0 # Biến lưu kết quả sau khi đảo bit

// for i in range(32): # Vì đề bài cho số 32-bit → lặp đúng 32 lần

// # B1: Dịch trái result 1 bit
// # -> Tạo chỗ trống ở cuối để gắn bit mới vào
// result = (result << 1)

// # B2: Lấy bit cuối của n (0 hoặc 1)
// # n & 1 sẽ trả về bit phải nhất của n
// last_bit = n & 1

// # B3: Gắn bit vừa lấy vào result
// # Dùng OR để thêm bit vào cuối
// result = result | last_bit

// # B4: Dịch phải n để xử lý bit tiếp theo
// n = n >> 1

// return result
// # ```

// # ---

// # ## 🔍 Giải thích từng phép toán bit

// # ### 1️⃣ `n & 1` — Lấy bit cuối

// # Ví dụ:

// # ```
// # n = 10110
// # &
// # 00001
// # = 00000 → bit cuối = 0
// # ```

// # Nếu:

// # ```
// # n = 10111
// # &
// # 00001
// # = 00001 → bit cuối = 1
// # ```

// # 👉 Dùng để “bóc” từng bit từ phải sang trái.

// # ---

// # ### 2️⃣ `result << 1` — Dịch trái

// # Ví dụ:

// # ```
// # result = 101
// # result << 1 → 1010
// # ```

// # Ý nghĩa:

// # * Nhân đôi số
// # * Tạo chỗ trống ở cuối để nhét bit mới vào

// # ---

// # ### 3️⃣ `result | last_bit` — Gắn bit vào

// # Ví dụ:

// # ```
// # result = 1010
// # last_bit = 1

// # 1010
// # OR 0001
// # = 1011
// # ```

// # ---

// # ## 🔄 Thuật toán hoạt động thế nào?

// # Ta đang làm đúng kiểu:

// # > Lấy bit cuối của n → gắn vào đầu result

// # Nhưng vì không thể “gắn đầu”, nên ta:

// # 1. Dịch trái result
// # 2. Nhét bit vào cuối

// # Lặp lại → sẽ thành đảo ngược.

// # ---

// ## 🧠 Trace ví dụ nhỏ (8 bit cho dễ nhìn)

// # Giả sử:

// # ```
// # n = 00010110
// # ```

// # | Lần | result (sau khi gắn) | n (sau shift) |
// # | --- | -------------------- | ------------- |
// # | 1 | 0 | 00001011 |
// # | 2 | 1 | 00000101 |
// # | 3 | 10 | 00000010 |
// # | 4 | 101 | 00000001 |
// # | 5 | 1011 | 00000000 |
// # | ... | ... | ... |

// # Cuối cùng:

// # ```
// # 01101000
// # ```

// # → Đã đảo bit.

// # ---

// # ## 📌 Tóm tắt thuật toán

// # **Input:** số 32 bit
// # **Process:**

// # Lặp 32 lần:

// # 1. Lấy bit cuối `n & 1`
// # 2. Dịch trái result
// # 3. Gắn bit vào result
// # 4. Dịch phải n

// # **Output:** result = số đã đảo bit

// # ---

// # ## 💡 Độ phức tạp

// # * Time: `O(32)` → coi như `O(1)`
// # * Space: `O(1)`

// # Vì số bit cố định.

// # ---

// # Nếu bạn muốn mình:

// # * Vẽ sơ đồ từng bước 32 bit
// # * So sánh với cách dùng string
// # * Giải thích vì sao phải lặp 32 thay vì đến khi n = 0

// # Cứ nói mình biết 👍

// # ---

// # ## 🧠 Ý nghĩa đề bài Reverse Bits

// # **Yêu cầu:**
// # Cho một số nguyên không dấu 32-bit `n`, hãy **đảo ngược thứ tự các bit**
// của nó và trả về số mới.

// # Nói đơn giản:

// # > Bit ở đầu → xuống cuối
// # > Bit ở cuối → lên đầu

// # ---

// # ## 📌 Ví dụ minh họa

// # ### Ví dụ 1

// # Input:

// # ```
// # n = 00000010100101000001111010011100
// # ```

// # Đây là biểu diễn **32 bit** của số nguyên.

// # Sau khi đảo bit:

// # ```
// # 00111001011110000010100101000000
// # ```

// # Output (dạng thập phân):

// # ```
// # 964176192
// # ```

// # ---

// # ## 👀 Hình dung trực quan

// # ---

// # ## 🔄 Nguyên lý đảo bit

// # Giả sử có chuỗi bit:

// # ```
// # abcde
// # ```

// # Đảo lại thành:

// # ```
// # edcba
// # ```

// # Với 32 bit cũng vậy — chỉ là dài hơn thôi.

// # ---

// # ## 🧮 Ví dụ nhỏ cho dễ hiểu (8 bit)

// # Giả sử:

// # ```
// # n = 00010110 (22)
// # ```

// # Đảo lại:

// # ```
// # 01101000 (104)
// # ```

// # ---

// # ## ⚙️ Cách máy tính làm

// # Thường làm theo quy trình:

// # 1. Lấy bit cuối của `n` (`n & 1`)
// # 2. Dịch trái kết quả (`res << 1`)
// # 3. Gắn bit vừa lấy vào
// # 4. Dịch phải `n` (`n >> 1`)
// # 5. Lặp 32 lần

// # ---

// # ## 📌 Tóm tắt đề bài

// # * Input: số nguyên **không dấu 32 bit**
// # * Task: đảo ngược thứ tự **từng bit**
// # * Output: số mới sau khi đảo

// # ---

// # Nếu bạn muốn, mình có thể:

// # * Giải thích code Java / C++ / Python
// # * Trace từng bước bằng ví dụ của bạn
// # * Giải thích vì sao phải dùng `<<` và `>>`

// # Cứ gửi code hoặc phần bạn chưa hiểu 👍
