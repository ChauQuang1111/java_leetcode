
// // # # Meeting Rooms III(27/12/2025)
// // # Rất tốt 👍
// // # Đây là **lời giải chuẩn contest** cho **Meeting Rooms III (LeetCode 2402)**.
// // # Mình sẽ làm 2 việc:
import java.util.*;

public class b130 {
    static Scanner sc = new Scanner(System.in);

    public static void main(String[] args) {
        // Nhập số phòng
        int n = sc.nextInt();

        // Nhập số meeting
        int m = sc.nextInt();

        // Nhập danh sách meetings
        int[][] meetings = new int[m][2];
        for (int i = 0; i < m; i++) {
            meetings[i][0] = sc.nextInt(); // start
            meetings[i][1] = sc.nextInt(); // end
        }

        int result = mostBooked(n, meetings);

        // In kết quả
        System.out.println(result);

        sc.close();
    }

    /**
     * Hàm tìm phòng họp được sử dụng nhiều nhất
     * 
     * @param n        số phòng
     * @param meetings danh sách các meeting [start, end]
     * @return chỉ số phòng được dùng nhiều nhất
     */
    public static int mostBooked(int n, int[][] meetings) {

        // count[i]: số lần phòng i được sử dụng
        int[] count = new int[n];

        // busy[i]: thời điểm phòng i sẽ rảnh
        // ban đầu tất cả = 0 (tất cả phòng đều trống)
        long[] busy = new long[n];

        // Sắp xếp meetings theo thời gian bắt đầu tăng dần
        Arrays.sort(meetings, (a, b) -> a[0] - b[0]);

        // Duyệt từng meeting theo thứ tự thời gian
        for (int[] meeting : meetings) {
            int start = meeting[0];
            int end = meeting[1];

            // earliest: thời điểm rảnh sớm nhất trong các phòng
            long earliest = Long.MAX_VALUE;

            // roomIndex: phòng tương ứng với earliest
            int roomIndex = -1;

            // assigned: đánh dấu xem đã gán được phòng hay chưa
            boolean assigned = false;

            // Duyệt tất cả các phòng
            for (int i = 0; i < n; i++) {

                // Cập nhật phòng kết thúc sớm nhất
                if (busy[i] < earliest) {
                    earliest = busy[i];
                    roomIndex = i;
                }

                // Nếu phòng i rảnh tại thời điểm start
                if (busy[i] <= start) {
                    // Gán meeting cho phòng i
                    busy[i] = end;
                    count[i]++;
                    assigned = true;

                    // break để đảm bảo chọn phòng có index nhỏ nhất
                    break;
                }
            }

            // Nếu không có phòng nào trống tại start
            if (!assigned) {
                // Phải chờ phòng kết thúc sớm nhất
                long duration = end - start;
                busy[roomIndex] += duration;
                count[roomIndex]++;
            }
        }

        // Tìm phòng được sử dụng nhiều nhất
        int max = 0;
        int res = 0;
        for (int i = 0; i < n; i++) {
            if (count[i] > max) {
                max = count[i];
                res = i;
            }
        }

        return res;
    }
}

// Mình sẽ **giải thích thuật toán của đoạn Java code này một cách rõ ràng**,
// tập trung vào **ý tưởng + luồng xử lý**, và chỉ ra **điểm mạnh / điểm yếu**
// so với lời giải chuẩn dùng heap.

// ---

// # 2402. Meeting Rooms III – Giải thích thuật toán trong code của bạn

// ## 1️⃣ Tóm tắt nhanh bài toán

// * Có `n` phòng họp (0 → n−1)
// * Mỗi meeting `[start, end]`
// * Quy tắc:

// 1. Nếu có phòng trống tại `start` → dùng **phòng index nhỏ nhất**
// 2. Nếu không có phòng trống → **chờ phòng kết thúc sớm nhất**, dời meeting
// (giữ nguyên thời lượng)
// * Đếm số lần mỗi phòng được dùng
// * Trả về phòng được dùng nhiều nhất (nếu hoà → index nhỏ hơn)

// ---

// ## 2️⃣ Các biến trong code và ý nghĩa

// ```java
// int[] count = new int[n];
// ```

// * `count[i]`: số meeting được gán cho phòng `i`

// ```java
// long[] busy = new long[n];
// ```

// * `busy[i]`: **thời điểm phòng i rảnh**
// * Ban đầu = 0 → tất cả phòng trống từ thời điểm 0

// ---

// ```java
// Arrays.sort(meetings, (a, b) -> a[0] - b[0]);
// ```

// * Sắp xếp meetings theo **start time tăng dần**
// * Bắt buộc để mô phỏng đúng theo thời gian

// ---

// ## 3️⃣ Ý tưởng thuật toán (cốt lõi)

// 👉 Với **mỗi meeting**, ta:

// 1. Quét qua **tất cả các phòng**
// 2. Tìm:

// * Phòng nào **đang trống sớm nhất**
// * Phòng nào **trống tại thời điểm start**

// ⚠️ Không dùng heap, mà **duyệt tuyến tính n phòng cho mỗi meeting**

// ---

// ## 4️⃣ Phân tích vòng lặp chính

// ```java
// for (int[] meeting : meetings) {
// ```

// ### Lấy thời gian meeting

// ```java
// int start = meeting[0], end = meeting[1];
// ```

// ---

// ### Biến phụ trợ

// ```java
// long earliest = Long.MAX_VALUE;
// int roomIndex = -1;
// boolean assigned = false;
// ```

// * `earliest`: thời điểm rảnh sớm nhất trong các phòng
// * `roomIndex`: phòng tương ứng với `earliest`
// * `assigned`: đã gán được phòng hay chưa

// ---

// ## 5️⃣ Vòng for duyệt từng phòng

// ```java
// for (int i = 0; i < n; i++) {
// ```

// ### 5.1 Tìm phòng rảnh sớm nhất

// ```java
// if (busy[i] < earliest) {
// earliest = busy[i];
// roomIndex = i;
// }
// ```

// 👉 Dù phòng có trống hay không, ta vẫn ghi nhận:

// * phòng kết thúc sớm nhất (để dùng nếu **phải chờ**)

// ---

// ### 5.2 Nếu phòng trống tại thời điểm start

// ```java
// if (busy[i] <= start) {
// busy[i] = end;
// count[i]++;
// assigned = true;
// break;
// }
// ```

// 👉 Nếu phòng `i` rảnh:

// * Gán meeting ngay cho phòng `i`
// * Cập nhật:

// * thời điểm bận mới = `end`
// * tăng `count`
// * `break` để đảm bảo:

// * **chọn phòng có index nhỏ nhất** (do duyệt từ 0 → n-1)

// ---

// ## 6️⃣ Nếu không có phòng trống

// ```java
// if (!assigned) {
// busy[roomIndex] += (end - start);
// count[roomIndex]++;
// }
// ```

// 👉 Tất cả phòng đều bận:

// * Chọn phòng kết thúc sớm nhất (`roomIndex`)
// * Dời meeting:

// * thời lượng = `end - start`
// * thời điểm kết thúc mới = `busy + duration`
// * Tăng số lần dùng phòng đó

// ---

// ## 7️⃣ Sau khi xử lý tất cả meetings

// ```java
// int max = 0, res = 0;
// for (int i = 0; i < n; i++) {
// if (count[i] > max) {
// max = count[i];
// res = i;
// }
// }
// ```

// * Tìm phòng được dùng nhiều nhất
// * Nếu hoà → giữ phòng có index nhỏ hơn (do duyệt từ trái sang)

// ---

// ## 8️⃣ Độ phức tạp

// ### ⏱ Time complexity

// * Sort meetings: `O(m log m)`
// * Với mỗi meeting, duyệt `n` phòng: `O(m × n)`

// 👉 **Tổng: O(mn)**
// ⚠️ Có thể **TLE nếu n và m lớn**

// ---

// ### 💾 Space complexity

// * `O(n)` cho `count` và `busy`

// ---

// ## 9️⃣ So sánh với lời giải chuẩn (2 heap)

// | Tiêu chí | Code hiện tại | Heap chuẩn |
// | --------- | ------------- | ------------ |
// | Dễ hiểu | ✅ | ⚠️ |
// | Hiệu năng | ❌ O(mn) | ✅ O(m log n) |
// | Contest | ❌ | ✅ |
// | Phỏng vấn | ⚠️ | ✅ |

// 👉 Code của bạn **đúng logic**, nhưng **không tối ưu** cho test lớn.

// ---

// ## 🔑 Kết luận (huấn luyện viên)

// * ✔ Ý tưởng mô phỏng **đúng đề**
// * ✔ Logic chọn phòng chính xác
// * ❌ Không dùng heap → **chậm**
// * 👉 Nếu n ≤ 100 → OK
// * 👉 Với constraint lớn → **nên dùng 2 priority queue**

// Nếu bạn muốn:

// * Mình **chuyển code này sang heap version (Java)**
// * So sánh **từng bước với heap**
// * Phân tích test khiến code này TLE

// 👉 cứ nói, mình hướng dẫn tiếp 💪

// # 1️⃣ **Giải thích thuật toán (ý tưởng + tư duy greedy)**
// # 2️⃣ **Viết lại code của bạn với chú thích chi tiết từng phần**

// # ---

// # # 1️⃣ Giải thích thuật toán (theo tư duy thi đấu)

// # ## Bài toán yêu cầu gì?

// # * Có `n` phòng họp (`0 → n-1`)
// # * Mỗi meeting có `[start, end]`
// # * Quy tắc:

// # * Nếu có phòng trống → chọn **phòng index nhỏ nhất**
// # * Nếu không có phòng trống → **chờ phòng kết thúc sớm nhất**
// # * Meeting bị dời nhưng **giữ nguyên thời lượng**
// # * Cuối cùng → trả về phòng được dùng nhiều nhất

// # ---

// # ## Insight quan trọng

// # Ta cần **luôn biết**:

// # * Phòng nào **đang trống** (lấy index nhỏ nhất)
// # * Phòng nào **kết thúc sớm nhất**

// # 👉 Không thể dùng mảng hay vòng lặp thường
// # 👉 **BẮT BUỘC dùng Priority Queue (heap)**

// # ---

// # ## 2 Heap cần thiết

// # ### 🔹 `avail` – phòng trống

// # * Min-heap theo **room index**
// # * Luôn lấy phòng nhỏ nhất

// # ### 🔹 `busy` – phòng đang bận

// # * Min-heap theo **(endTime, room)**
// # * Lấy phòng kết thúc sớm nhất

// # ---

// # ## Chiến lược Greedy

// # Duyệt từng meeting theo **start time tăng dần**:

// # ### Bước 1: Trả phòng đã xong

// # ```text
// # Nếu busyRoom.endTime <= start
// # → phòng đó rảnh → đưa về avail
// # ```

// # ---

// # ### Bước 2: Gán phòng

// # * Nếu `avail` không rỗng:

// # * Dùng phòng nhỏ nhất
// # * Nếu `avail` rỗng:

// # * Chờ phòng kết thúc sớm nhất
// # * Dời meeting sang thời điểm đó

// # ---

// # ### Bước 3: Cập nhật

// # * Đẩy phòng vào `busy`
// # * Tăng số lần dùng phòng

// # ---

// # # 2️⃣ Code có chú thích chi tiết

// # ```python
// from typing import List
// from heapq import heappush, heappop, heapify

// class Solution:
// def mostBooked(self, n: int, meetings: List[List[int]]) -> int:

// # Sắp xếp meetings theo thời gian bắt đầu
// meetings.sort(key=lambda x: x[0])

// # used[i]: số lần phòng i được sử dụng
// used = [0] * n

// # avail: min-heap lưu các phòng đang trống (theo index nhỏ nhất)
// avail = list(range(n))
// heapify(avail)

// # busy: min-heap lưu các phòng đang bận
// # mỗi phần tử là (endTime, room)
// busy = []
// heapify(busy)

// # Duyệt từng meeting
// for start, end in meetings:

// # Trả lại các phòng đã họp xong trước hoặc đúng start
// while busy and busy[0][0] <= start:
// _, room = heappop(busy)
// heappush(avail, room)

// # Nếu còn phòng trống
// if avail:
// room = heappop(avail)
// heappush(busy, (end, room))
// used[room] += 1

// # Nếu không còn phòng trống → phải chờ
// else:
// dur = end - start # thời lượng meeting
// endTime, room = heappop(busy) # phòng kết thúc sớm nhất
// heappush(busy, (endTime + dur, room))
// used[room] += 1

// # Tìm phòng được dùng nhiều nhất
// maxRoom = 0
// maxCount = 0
// for i in range(n):
// if used[i] > maxCount:
// maxCount = used[i]
// maxRoom = i

// # Trả về phòng được dùng nhiều nhất
// return maxRoom
// # ```

// # ---

// # # 3️⃣ Ví dụ minh hoạ ngắn

// # ```
// # n = 2
// # meetings = [[0,10],[1,5],[2,7],[3,4]]
// # ```

// # | Meeting | Hành động |
// # | ------- | ---------- |
// # | [0,10] | room 0 |
// # | [1,5] | room 1 |
// # | [2,7] | chờ room 1 |
// # | [3,4] | chờ room 0 |

// # → room 0 dùng nhiều nhất → trả về `0`

// # ---

// # # 4️⃣ Độ phức tạp

// # * Sort meetings: `O(m log m)`
// # * Heap operations: `O(m log n)`
// # * Tổng: **O(m log n)**
// # * Bộ nhớ: **O(n)**

// # ---

// # # 5️⃣ Ghi nhớ khi đi thi 🧠

// # > “Interval + phòng + sớm nhất / nhỏ nhất
// # > → **2 priority queue**”

// # ---

// # Nếu bạn muốn:

// # * Mình **giải thích từng dòng trong vòng while**
// # * Viết **Java version**
// # * So sánh với **Meeting Rooms I / II**
// # * Vẽ **timeline mô phỏng**

// # 👉 cứ nói, mình huấn luyện tiếp 💪

// # OK 👍
// # Mình sẽ **giải thích bài “Meeting Rooms III” (LeetCode 2402)** theo đúng
// phong cách **huấn luyện viên lập trình thi đấu**:
// # 👉 hiểu bài → mô hình hoá → cấu trúc dữ liệu → thuật toán chuẩn contest.

// # ---

// # ## 1️⃣ Tóm tắt đề bài

// # Bạn có:

// # * `n` phòng họp, đánh số `0 → n-1`
// # * `meetings[i] = [start, end]`

// # Quy tắc:

// # 1. Nếu có **phòng trống tại thời điểm start**
// # → dùng phòng có **chỉ số nhỏ nhất**
// # 2. Nếu **không có phòng trống**
// # → **chờ phòng kết thúc sớm nhất**, và **dời meeting** sang thời điểm đó
// # (giữ nguyên thời lượng)
// # 3. Mỗi meeting được gán đúng **1 phòng**
// # 4. Đếm số lần mỗi phòng được dùng

// # 🎯 **Trả về phòng được dùng nhiều nhất**
// # (Nếu hoà → lấy phòng có index nhỏ hơn)

// # ---

// # ## 2️⃣ Insight quan trọng (thi đấu)

// # 👉 Ta cần **mô phỏng theo thời gian**, và luôn phải biết:

// # * Phòng nào **đang trống**
// # * Phòng nào **sẽ rảnh sớm nhất**

// # ➡️ **Bài toán priority queue (heap)**

// # ---

// # ## 3️⃣ Hai heap bắt buộc phải có

// # ### 🔹 Heap 1: phòng trống

// # ```text
// # availableRooms (min-heap)
// # ```

// # * Lưu **chỉ số phòng**
// # * Luôn lấy phòng nhỏ nhất

// # ---

// # ### 🔹 Heap 2: phòng đang bận

// # ```text
// # busyRooms (min-heap)
// # ```

// # * Lưu `(endTime, roomIndex)`
// # * So sánh theo:

// # 1. `endTime` nhỏ nhất
// # 2. nếu bằng → `roomIndex` nhỏ hơn

// # ---

// # ## 4️⃣ Chiến lược thuật toán

// # ### Bước 1: Sort meetings

// # ```text
// # Sort theo start tăng dần
// # ```

// # ---

// # ### Bước 2: Khởi tạo

// # * Tất cả phòng `0 → n-1` vào `availableRooms`
// # * Mảng `count[n] = 0`

// # ---

// # ### Bước 3: Duyệt từng meeting

// # Giả sử meeting `[start, end]`, duration = `end - start`

// # #### 🔹 Trả phòng đã xong

// # ```text
// # Trong khi busyRooms.peek().endTime <= start:
// # trả phòng đó về availableRooms
// # ```

// # ---

// # #### 🔹 Nếu có phòng trống

// # ```text
// # room = availableRooms.pop()
// # endTime = end
// # ```

// # ---

// # #### 🔹 Nếu KHÔNG có phòng trống

// # ```text
// # (room, freeTime) = busyRooms.pop()
// # endTime = freeTime + duration
// # ```

// # ---

// # #### 🔹 Cập nhật

// # ```text
// # count[room]++
// # push (endTime, room) vào busyRooms
// # ```

// # ---

// # ## 5️⃣ Vì sao greedy này đúng?

// # * Meeting **không được bỏ**
// # * Nếu có phòng trống → dùng ngay phòng nhỏ nhất
// # * Nếu không → chờ phòng xong sớm nhất để **giảm delay tổng thể**

// # 👉 Đây là **mô phỏng đúng đề**, không có trick ẩn

// # ---

// # ## 6️⃣ Ví dụ ngắn

// # ```
// # n = 2
// # meetings = [[0,10],[1,5],[2,7],[3,4]]
// # ```

// # * meeting[0]: room 0
// # * meeting[1]: room 1
// # * meeting[2]: room 1 (dời)
// # * meeting[3]: room 0 (dời)

// # 👉 room 0 dùng nhiều nhất → trả về 0

// # ---

// # ## 7️⃣ Code mẫu (Python – chuẩn thi đấu)

// # ```python
// # import heapq

// # class Solution:
// # def mostBooked(self, n: int, meetings):
// # meetings.sort()

// # available = list(range(n)) # phòng trống
// # heapq.heapify(available)

// # busy = [] # (endTime, room)
// # count = [0] * n

// # for start, end in meetings:
// # duration = end - start

// # # trả phòng đã xong
// # while busy and busy[0][0] <= start:
// # _, room = heapq.heappop(busy)
// # heapq.heappush(available, room)

// # if available:
// # room = heapq.heappop(available)
// # finish = end
// # else:
// # finish, room = heapq.heappop(busy)
// # finish += duration

// # count[room] += 1
// # heapq.heappush(busy, (finish, room))

// # return count.index(max(count))
// # ```

// # ---

// # ## 8️⃣ Độ phức tạp

// # * Sort meetings: `O(m log m)`
// # * Mỗi meeting push/pop heap: `O(log n)`

// # 👉 Tổng: **O(m log n)**
// # 👉 Bộ nhớ: **O(n)**

// # ---

// # ## 9️⃣ Dấu hiệu nhận dạng bài này trong contest

// # Nếu bạn thấy:

// # * meeting / task / interval
// # * phòng / server / máy
// # * dùng “sớm nhất” / “nhỏ nhất”

// # 👉 **Priority Queue 2 heap**

// # ---

// # Nếu bạn muốn:

// # * Giải thích **từng dòng code**
// # * Viết **Java version**
// # * So sánh với **Meeting Rooms I / II**
// # * Vẽ **timeline mô phỏng**

// # 👉 cứ nói, mình huấn luyện tiếp 💪
