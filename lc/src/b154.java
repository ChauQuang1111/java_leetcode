...
// Mình sẽ**giữ nguyên logic của bạn**,chỉ:

// 1.**Thêm hàm`main`dùng`Scanner`**2.**Chú thích chi tiết từng phần trong code**3. Viết theo phong cách**dễ học–đúng chuẩn phỏng vấn/LeetCode**
...
public class b154 {
    ...
    /**
     * Hàm chính giải bài 3314 - Construct the Minimum Bitwise Array I
     * 
     * @param nums danh sách số nguyên đầu vào
     * @return mảng kết quả nhỏ nhất theo yêu cầu đề bài
     */
    public static int[] minBitwiseArray(List<Integer> nums) {
        int[] res = new int[nums.size()]; // mảng kết quả

        for (int i = 0; i < nums.size(); i++) {
            int n = nums.get(i);

            // Nếu n là số chẵn → không tồn tại nghiệm
            if ((n & 1) == 0) {
                res[i] = -1;
            } else {
                /*
                 ...
                 *
                 * >> 1 :
                 * - dịch phải để xác định bit 1 cần tắt
                 *
                 * n & ~(...) :
                 * - tắt bit đó để tạo ans[i] nhỏ nhất
                 */
                res[i] = n & ~(((n + 1) & ~n) >> 1);
            }
        }
...
// tư duy (để nhớ khi đi thi)

// * ❌ **Số chẵn → -1**
// * ✅ **Số lẻ**:

// * Tìm bit 0 thấp nhất: `(n + 1) & ~n`
// * Dịch phải 1 bit
// * Tắt bit đó để tạo số nhỏ nhất

// ---

// Nếu bạn muốn mình:

// * ✍️ Viết lại **dạng while (theo đúng sở thích của bạn)**
// * 🧩 Giải thích **bằng bảng bit từng bước**
// * 🔁 So sánh với cách brute force
// * 🎯 Viết version **chuẩn LeetCode submission**

// 👉 cứ nói tiếp nhé, mình hỗ trợ tới cùng 👌
...
// # mà OR của hai số **luôn ≥ số lớn hơn**, và để đạt **giá trị chẵn**, cả hai
// số phải có bit cuối = `0` → mâu thuẫn với yêu cầu tối thiểu
// # ➜ trả về `-1`
...
// # Nhiệm vụ của bạn là **xây dựng một mảng mới** `ans` cũng có độ dài `n` sao
// cho:
...
// # ➡️ **chọn mảng có giá trị nhỏ nhất theo thứ tự từ trái sang phải
