...
// Dưới đây là**phiên bản đầy đủ có`main`dùng`Scanner`+chú thích chi tiết từng phần**cho code Java của bạn.Mình viết theo**đúng phong cách học thuật+dễ debug khi chạy local**.

// ---

// ##🧠Ý tưởng thuật toán(nhắc nhanh)

// *`getSum()`→tính**tổng toàn bộ cây***`getMaxProduct()`→DFS hậu tố,với mỗi node:

// *coi như**cắt cạnh phía trên node***tính tích:

// ```subtreeSum*(totalSum-subtreeSum)```*Lấy tích lớn nhất,cuối cùng`%(10^9+7)`

// ---

// ##✅Code hoàn chỉnh(có`main`,dùng`Scanner`,có chú thích)
...
public class b139 {
    ...
    public static void main(String[] args) {
        ...
        // Tạo cây mẫu (hard-code cho dễ test)
        ...
        root.right.right = new TreeNode(arr[5]);
     ...
     * Trả về: tổng cây con tại node hiện tại
     */
    public static long getMaxProduct(TreeNode root) {
        if (root == null) {
            return 0;
        }
        ...
        // Giả sử cắt cạnh phía trên node này
        long product = subtreeSum * (sum - subtreeSum);
        ...
        // Trả tổng cây con cho node cha
        return subtreeSum;
    }

    /**
     * Hàm theo yêu cầu đề bài LeetCode
     */
    public static int maxProduct(TreeNode root) {
        ...
        // Bước 2: DFS để tìm tích lớn nhất
        getMaxProduct(root);

        // Bước 3: trả kết quả modulo
...
// quan trọng (rất hay bị hỏi)

// 1. **Không modulo khi so sánh**

// * Chỉ `% MODULO` khi return cuối
// 2. **Bắt buộc dùng `long`**

// * Vì tích có thể vượt `int`
// 3. DFS phải là **post-order**
// 4. `static` dùng cho LeetCode là OK, nhưng:

// * Khi thi / project → nên tránh
...
// * 🔁 Viết **phiên bản không dùng biến static**
// * 🌳 Viết **hàm build tree từ input level-order**
// * 🧠 Vẽ **trace DFS từng node**
// * 🎯 Chỉ ra **điểm ghi điểm khi phỏng vấn**
...
// # * Với mỗi `s` trong `subtree_sums`, giả sử **cắt cạnh phía trên cây con
// đó**
