...
public class b216 {
    ...
    // Các ký tự được phép dùng
    public static char[] ALLOWED_LETTERS = { 'a', 'b', 'c' };

    // Đếm số happy string đã tạo
    public static int happyStringCount;

    // Lưu chuỗi happy thứ k
    public static String kthHappyString;

    // Hàm chính để tìm chuỗi happy thứ k
    public static String getHappyString(int n, int k) {
        ...
        // nếu không đủ k chuỗi thì trả về ""
        return happyStringCount >= k ? kthHappyString : "";
    }

    // Hàm DFS / Backtracking
    public static void kthHappyStringHelper(StringBuilder currStr, int n, int k) {

        // độ dài chuỗi hiện tại
        int currStrSize = currStr.length();

        // nếu đạt độ dài n → tìm được 1 happy string
        if (currStrSize == n) {

            happyStringCount++; // tăng biến đếm

            // nếu đây là chuỗi thứ k
        ...
        for (char c : ALLOWED_LETTERS) {
            ...
            // nếu ký tự giống ký tự trước → bỏ qua
            ...
            // gọi đệ quy để tiếp tục xây chuỗi
            kthHappyStringHelper(currStr, n, k);

            // backtrack: xóa ký tự vừa thêm
            currStr.deleteCharAt(currStrSize);

            // nếu đã tìm được chuỗi thứ k thì dừng luôn
            if (happyStringCount == k)
                return;
        }
    }
...
// Dưới đây là**giải thích thuật toán+pseudocode+code Java
// có`main`dùng`Scanner`và chú thích chi tiết**cho bài LeetCode 1415:The k-th
...
// Ta cần tìm**chuỗi happy thứ k**theo**thứ tự từ điển**.

// ##Happy string là gì

// Chuỗi:

// *chỉ gồm`a,b,c`***hai ký tự liền nhau không giống nhau**
...
// #2. Cách giải(Backtracking/DFS)

// Ta**tạo tất cả chuỗi hợp lệ theo thứ tự từ điển**.

// Thứ tự thử ký tự:

// ```a→b→c```

// Nhờ vậy các chuỗi sinh ra**tự động đúng thứ tự từ điển**.

// Quy trình:

// 1. bắt đầu chuỗi rỗng 2. thử thêm`a`,`b`,`c`3. nếu trùng ký tự trước→bỏ qua
// 4. nếu độ dài=`n`→tìm được 1 happy string 5. tăng biến đếm 6. nếu đếm=`k`→lưu
// kết quả
...
// if count>=k return result else return""

// function DFS(currentString):

// if length(currentString)==n count++if count==k result=currentString return

// for each char in['a','b','c']

// if last char of currentString==char continue

// add char to currentString

// DFS(currentString)

// remove last char(backtrack)

// if count==k stop recursion```

// ---

// #4. Code Java đầy đủ(có`main`+`Scanner`+chú thích)
...
// ```1 aba 2 abc 3 aca 4 acb 5 bab 6 bac 7 bca 8 bcb 9 cab 10 cac 11 cba 12
// cbc```
...
// Nhưng thực tế **dừng sớm khi tìm được k** nên thường nhanh hơn.

// ---

// ✅ Nếu bạn muốn, tôi có thể giải thích thêm **cây recursion của bài

// này (rất dễ hiểu, nhiều người dùng để phỏng vấn Google / Amazon)**.

// # # Bài **“The k-th Lexicographical String of All Happy Strings of Length
// n”** (trên LeetCode 1415: The k-th Lexicographical String of All Happy
// Strings of Length n) có ý nghĩa như sau:

// # Thuật toán này **không sinh toàn bộ chuỗi** (tránh backtracking), mà **đếm
// số lượng chuỗi có thể tạo ở mỗi nhánh rồi bỏ qua cả một “block”** nếu `k` lớn
// hơn số chuỗi trong block đó. Nhờ vậy chạy **O(n)**.
...
// # Nếu bạn muốn, tôi có thể giải thích thêm **vì sao `cnt = 2^(remaining_len)`
// đúng** (đây là chỗ quan trọng nhất của bài này).
...
// # > Tạo tất cả chuỗi độ dài **n** từ `{a,b,c}` sao cho **không có hai ký tự
// liền nhau giống nhau**, sắp xếp theo **thứ tự từ điển**, rồi trả về **chuỗi
// đứng thứ k**.
