...
// Ok 👍 mình giải thích **thuật toán của đoạn Java code này theo đúng tư duy đề “Divide an Array Into Subarrays With Minimum Cost I”**, ngắn gọn nhưng **đủ sâu để nhớ lâu**.
import java.util.*;
public class b166{
   ...
   public static void main(String[] args) {
        ...
        // Gọi hàm minimumCost và in kết quả
...
// Hàm giải bài toán
    public static int minimumCost(int[] nums) {
        ...
        // nên chắc chắn được cộng vào tổng cost
        int first = nums[0];

        // min1: số nhỏ nhất trong nums[1...]
        // min2: số nhỏ thứ hai trong nums[1...]
        ...
        for (int i = 1; i < nums.length; i++) {
            ...
            // Nếu tìm được số nhỏ hơn min1
            if (nums[i] < min1) {
                // Đẩy min1 cũ xuống min2
                min2 = min1;
                min1 = nums[i];

            // Nếu nums[i] không nhỏ hơn min1
            // nhưng nhỏ hơn min2
            } else if (nums[i] < min2) {
                min2 = nums[i];
            }
...
// ## 🧠 Tư duy cốt lõi của bài

// ### Điều kiện đề bài (bản I):

// * `k = 2` ⇒ **mỗi subarray phải có ít nhất 2 phần tử**
// * Subarray đầu tiên **bắt buộc chứa `nums[0]`**

// ### Cost:

// * **Cost của mỗi subarray = phần tử đầu tiên của subarray**

// ### Mục tiêu:

// ➡️ **Tổng cost nhỏ nhất**

// ---

// ## 🎯 Suy luận quan trọng

// 1. Vì `nums[0]` **bắt buộc** nằm trong subarray đầu tiên
//    👉 `nums[0]` **luôn được cộng vào kết quả**

// 2. Vì mỗi subarray cần **ít nhất 2 phần tử**, nên:

//    * Ngoài subarray đầu tiên, ta cần **2 subarray nữa**
//    * Mỗi subarray mới cần **1 phần tử làm “đầu subarray”**

// 3. Để tổng cost **nhỏ nhất**:
//    👉 Chọn **2 số nhỏ nhất** trong các phần tử còn lại (`nums[1…n-1]`)

// 💡 **Không cần thật sự chia mảng**, chỉ cần chọn đúng các giá trị cộng vào cost.
...
//         // nums[0] bắt buộc thuộc subarray đầu tiên
//         // nên cost chắc chắn có nums[0]
//         int first = nums[0];

//         // Hai biến lưu 2 số nhỏ nhất trong nums[1...]
//         int min1 = Integer.MAX_VALUE;
//         int min2 = Integer.MAX_VALUE;
// ```
...
// 👉 Duyệt các phần tử **từ vị trí 1 trở đi**, vì `nums[0]` đã dùng rồi
...
// * `first` → subarray đầu tiên
// * `min1`, `min2` → đầu của 2 subarray còn lại
...
// ## 🧠 Tóm tắt 1 dòng (rất hay dùng khi phỏng vấn)

// > Vì `k = 2`, ta chỉ cần cộng `nums[0]` và **2 số nhỏ nhất trong phần còn lại** để được tổng cost nhỏ nhất.

// ---

// Nếu bạn muốn:

// * ✍️ Viết **comment chuẩn LeetCode**
// * 🔁 So sánh với **Minimum Cost II (k bất kỳ)**
// * 🧪 Phân tích **case biên**
