...
// ##🔥761. Special Binary String—Giải thích thuật toán(Java)

// Bản chất bài này:

// *Special Binary String giống**ngoặc hợp lệ***Luôn có dạng:

// ```1+(special string bên trong)+0```

// *Ta tách các special substring độc lập*Đệ quy xử lý bên trong*Sắp xếp giảm dần để tạo chuỗi lớn nhất theo thứ tự từ điển

// ---

// #🧠Giải thích thuật toán trong code bạn đưa

// Ý tưởng chính:

// 1. Dùng biến`acc`để theo dõi độ cân bằng

// *gặp`'1'`→`+1`*gặp`'0'`→`-1`

// 2. Khi`acc==0`→ta tìm được một special substring

// 3. Gọi đệ quy xử lý substring đó

// 4. Đưa vào`PriorityQueue`để tự động sắp xếp giảm dần

// 5. Ghép lại kết quả

// ---

// #📌Code đầy đủ(có main+Scanner+chú thích chi tiết)
...
public class b185 {
    ...
    public static void main(String[] args) {
        ...
        String result = makeLargestSpecial(input);
    ...
    public static String makeLargestSpecial(String S) {
        ...
        // Nếu chuỗi null hoặc độ dài <= 2 thì không thể tách nhỏ hơn
        // Ví dụ: "10" là special nhỏ nhất
        if (S == null || S.length() <= 2) {
            return S;
        }

        // PriorityQueue để lưu các special substring
        // Sắp xếp giảm dần (chuỗi lớn hơn đứng trước)
        PriorityQueue<String> pq = new PriorityQueue<>(
                (a, b) -> b.compareTo(a));

        // acc dùng để theo dõi cân bằng
        // Ban đầu = 1 vì S.charAt(0) luôn là '1'
        int acc = 1;

        // prev đánh dấu vị trí bắt đầu của special substring
        int prev = 0;

        // Duyệt từ vị trí 1
        for (int i = 1; i <= S.length(); i++) {

            // Nếu acc == 0 nghĩa là từ prev → i-1 là special substring
            if (acc == 0) {

                // Nếu không phải toàn bộ chuỗi
                if (!(prev == 0 && i == S.length())) {

                    // Gọi đệ quy xử lý phần này
                    String sub = makeLargestSpecial(S.substring(prev, i));

                    // Thêm vào PriorityQueue
                    pq.add(sub);
                }

                // Cập nhật prev để tìm special substring tiếp theo
                prev = i;
            }

            // Nếu đã tới cuối chuỗi thì dừng
            if (i == S.length()) {
                break;
            }
        ...
        // Ghép các substring đã sort giảm dần
        ...
        // Nếu pq rỗng nghĩa là chuỗi có dạng:
        // 1 + (special bên trong) + 0
        // Ví dụ: 1100 hoặc 111000
        if (ans.length() == 0) {

            ans.append('1');

            // Xử lý phần bên trong (bỏ 1 và 0 ngoài cùng)
            ans.append(
                    makeLargestSpecial(
...
// ```11011000=1(101100)0=1(110010)0=11100100```
...
// ```
// java Collections.sort(list,Collections.reverseOrder());```
...
// | Python | Java |
// | ------------------------- | -------------------------- |
// | list + sort(reverse=True) | PriorityQueue max heap |
// | logic rõ ràng | viết hơi phức tạp hơn |
// | dễ đọc hơn | tối ưu hơn khi nhiều chunk |

// ---

// Nếu bạn muốn mình:

// * 🔬 Trace từng vòng lặp bằng ví dụ cụ thể
// * 🧠 Giải thích tại sao greedy + sort là đúng
// * 📊 So sánh với cách dùng ArrayList thay vì PQ
// * 💡 Viết lại phiên bản Java dễ hiểu hơn

// Bạn muốn đào sâu phần nào?

// # Rất tốt 👌 Đây chính là lời giải chuẩn của bài **Special Binary String
// (LeetCode 761)**.

// # Mình sẽ **viết lại code + thêm chú thích chi tiết từng dòng** để bạn hiểu
// bản chất thuật toán (đệ quy + greedy + sort).
...
// # Một chuỗi nhị phân (chỉ gồm `'0'` và `'1'`) được gọi là **special** nếu
...
// # Sau khi xử lý và sắp xếp lại các special substring con theo thứ tự giảm
// dần:
