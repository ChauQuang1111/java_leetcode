// // public # ## 📌 Binary Gap (22/02/2026)

// ##🔎Giải thích thuật toán(Java–LeetCode 868)

// Bài**Binary Gap(868)**yêu cầu:

// >Tìm**khoảng cách lớn nhất giữa hai bit 1 liên tiếp**trong biểu diễn nhị phân của số`n`.

// 📌Khoảng cách=hiệu vị trí giữa hai bit 1 gần nhau nhất(không phải chỉ đếm số 0).

// ---

// ##🧠Ý tưởng chính

// ###1 ️⃣Kiểm tra số có phải lũy thừa của 2 không

// ```java if((n&(n-1))==0)return 0;```

// *Nếu n chỉ có**1 bit 1**(ví dụ 8=1000)*Thì không có 2 bit 1 nào để tính khoảng cách*→Trả về 0

// ---

// ###2 ️⃣Bỏ các số 0 phía sau

// ```java n>>=Integer.numberOfTrailingZeros(n);```

// Ví dụ:

// ```n=20 Binary:10100```

// *Có 2 số 0 ở cuối*`Integer.numberOfTrailingZeros(n)`=2*Dịch phải 2 bit→còn`101`
...
// *Cập nhật khoảng cách lớn nhất*Reset biến đếm*Nếu gặp`0`:

// *Tăng biến đếm

// ---

// ##✅Code đầy đủ(có chú thích+hàm main dùng Scanner)
...
public class b187 {
    ...
    public static int binaryGap(int n) {
        ...
        // Nếu n là lũy thừa của 2 (chỉ có 1 bit 1)
        // Ví dụ: 8 = 1000, 16 = 10000
        // Không có 2 bit 1 → return 0
        if ((n & (n - 1)) == 0)
            return 0;

        // Bỏ các số 0 ở cuối (dịch phải số lượng trailing zeros)
        // Ví dụ: 20 = 10100
        // trailing zeros = 2
        // sau khi dịch: 101
        n >>= Integer.numberOfTrailingZeros(n);

        int maxGap = 0; // khoảng cách lớn nhất
        int gap = 0; // khoảng cách hiện tại giữa 2 bit 1
        ...
        while (n > 0) {
            ...
            if ((n & 1) == 1) {
                ...
                // Nếu bit là 0 thì tăng khoảng cách
                gap++;
            }
        ...
        // nhưng đề yêu cầu khoảng cách vị trí
        return maxGap + 1;
    }
}
...
// * Viết phiên bản dùng while thuần (không dùng numberOfTrailingZeros)
// * Hoặc giải thích sâu về `(n & -n)`
// * Hoặc vẽ sơ đồ bit minh họa từng bước

// Nói mình biết nhé 🚀

// # Bài **868. Binary Gap** yêu cầu:

// # > Tìm khoảng cách lớn nhất giữa **hai bit 1 liên tiếp** trong biểu diễn nhị
// phân của số `n`.

// # ⚠️ Lưu ý:
// # Binary Gap trong bài 868 = **khoảng cách giữa hai bit 1 liên tiếp** (tính
// cả vị trí),
...
// # > **Số lượng số 0 liên tiếp dài nhất nằm giữa hai số 1 trong biểu diễn nhị
// phân của một số nguyên dương.**
...
// # Nếu bạn muốn mình giải thích luôn cách code (Java / Python) hoặc phân tích
// độ phức tạp thì nói mình biết nhé 🚀
// b187 {
